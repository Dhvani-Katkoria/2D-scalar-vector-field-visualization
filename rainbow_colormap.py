    ...
    for indexX in range(image.shape[0]):
        for indexY in range(image.shape[1]):
            if (aggData[i][indexX][indexY] > xmaxd):
                colorPoint = np.add(np.multiply((1 - alpha), colorOrangeRed), np.multiply(alpha, colorRed))
            else:
                alpha = ((aggData[i][indexX][indexY]) - xmind) / (xmaxd - xmind)
                if (alpha < (1 / 6)):
                    colorPoint = np.add(np.multiply((1 - alpha), colorViolet), np.multiply(alpha, colorIndigo))
                elif (alpha < (2 / 6)):
                    colorPoint = np.add(np.multiply((1 - alpha), colorIndigo), np.multiply(alpha, colorBlue))
                elif (alpha < (3 / 6)):
                    colorPoint = np.add(np.multiply((1 - alpha), colorBlue), np.multiply(alpha, colorGreen))
                elif (alpha < (4 / 6)):
                    colorPoint = np.add(np.multiply((1 - alpha), colorGreen), np.multiply(alpha, colorYellow))
                elif (alpha < (5 / 6)):
                    colorPoint = np.add(np.multiply((1 - alpha), colorYellow), np.multiply(alpha, colorOrange))
                else:
                    colorPoint = np.add(np.multiply((1 - alpha), colorOrange), np.multiply(alpha, colorOrangeRed))
